...
package com.chilliwifi.you2b.searchyou2b.rx;
...
public abstract class MvpLceRxPresenter<V extends MvpLceView<M>, M>
    ...
    public void subscribe(Observable<M> observable, final boolean pullToRefresh) {
        ...
        if (isViewAttached()) {
            System.out.println("Javier show loading " + observable);
        ...
        subscriber = new Subscriber<M>() {
            ...
            public void onCompleted() {
                System.out.println("Javier on completed");
        ...
        observable.subscribeOn(getSubscribeOnScheduler()).observeOn(getObserveOnScheduler()).subscribe(subscriber);

    }

    private Scheduler getSubscribeOnScheduler() {
        return isTestMode ? Schedulers.immediate() : Schedulers.io();
    }

    private Scheduler getObserveOnScheduler() {
        return isTestMode ? Schedulers.immediate() : AndroidSchedulers.mainThread();
    }
    ...
    protected void onError(Throwable e, boolean pullToRefresh) {
        if (isViewAttached()) {
            System.out.println("Javier throwable e " + e.toString());
